...
namespace CelestialLeague.Client.Networking
{
    public class ConnectionManager : IDisposable
    {
        ...
        where TRequest : BasePacket where TResponse : BasePacket
        {
            ...
            var timeoutMs = (int)(timeout?.TotalMilliseconds ?? NetworkConstants.SocketTimeoutMs);
        ...
        private async Task HeartbeatLoopAsync(CancellationToken cancellationToken)
        {
            while (!cancellationToken.IsCancellationRequested && IsConnected)
            {
                try
                {
                    await Task.Delay(NetworkConstants.HeartbeatIntervalMs, cancellationToken);
                    ...
                    if (timeSinceLastActivity > TimeSpan.FromMilliseconds(NetworkConstants.ConnectionTimeoutMs))
                    {
        ...
        {
            var cutoffTime = DateTime.UtcNow.AddSeconds(-NetworkConstants.SocketTimeoutMs);
