...
public class InterfaceManager : Entity
{
    ...
    public override void Added(Scene scene)
    {
        ...
        Logger.Log(LogLevel.Info, "CelestialLeague", "InterfaceManager initialized");
    }
    ...
    public void AddChild(IUIComponent child) => rootContainer.AddChild(child);
    ...
    // Input helper methods that your UIComponent.Update() expects
    ...
    // Legacy input methods for backward compatibility
    ...
    public void DrawText(string text, Vector2 position, Color color, float scale = 0.75f)
    {
        if (string.IsNullOrEmpty(text)) return;

        if (Dialog.Languages != null && Dialog.Languages.ContainsKey("english"))
        {
            ...
            for (int i = 0; i < text.Length; i++)
            {
                ...
                {
                    offset.X = 0;
                    offset.Y += size.LineHeight * scale;
                    continue;
                }
                ...
                if (fontChar != null)
                {
                    var charPos = position + offset + new Vector2(fontChar.XOffset * scale, fontChar.YOffset * scale);
                    SpriteBatch.Draw(
                        fontChar.Texture.Texture.Texture_Safe,
                        charPos,
                        fontChar.Texture.ClipRect,
                        color,
                        0f,
                        Vector2.Zero,
                        scale,
                    ...
                    offset.X += fontChar.XAdvance * scale;
                }
            }
        }
    }

    public Vector2 MeasureText(string text, float scale = 0.75f)
    {
        ...
        if (Dialog.Languages != null && Dialog.Languages.ContainsKey("english"))
        {
            ...
            for (int i = 0; i < text.Length; i++)
            {
                ...
                if (fontChar != null)
                {
                    currentWidth += fontChar.XAdvance * scale;
                }
            }
            ...
            return new Vector2(widest, lines * size.LineHeight * scale);
        }
        return Vector2.Zero;
    }
