    ...
    public class NetworkClient : IDisposable
    {
        ...
        public async Task<bool> SendPacketAsync<T>(T packet, NetworkStream stream) where T : BasePacket
        {
            ...
            try
            {
                ...
                if (packetData.Length > NetworkConstants.MaxPacketSize)
                {
                    Logger.Error("CelestialLeague", $"Packet too large: {packetData.Length} bytes (max: {NetworkConstants.MaxPacketSize})");
                    SendErrors++;
                    return false;
                }
