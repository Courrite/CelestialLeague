namespace CelestialLeague.Shared.Utils
{
    public class TimeHelpers
    {
        ...
        {
            if (seasonNumber < 1) return GameConstants.FirstSeasonStart;
            return GameConstants.FirstSeasonStart.AddTicks((seasonNumber - 1) * GameConstants.SeasonDuration.Ticks);
        }

        public static DateTime GetSeasonEnd(int seasonNumber)
        {
            return GetSeasonStart(seasonNumber).Add(GameConstants.SeasonDuration);
        }

        public static int GetCurrentSeason()
        {
            var elapsed = DateTime.UtcNow - GameConstants.FirstSeasonStart;
            return Math.Max(1, (int)(elapsed.Ticks / GameConstants.SeasonDuration.Ticks) + 1);
        }
