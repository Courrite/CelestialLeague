...
namespace CelestialLeague.Server.Utils
{
    public class PacketProcessor
    {
        ...
        public PacketProcessor(GameServer gameServer, BaseHandler[] handlers)
        {
            ...
            Logger.Info($"PacketProcessor initialized with {_handlers.Count} handlers");
        }
        ...
        public async Task ProcessAsync(ClientConnection connection, BasePacket packet)
        {
            ...
            {
                Logger.Warning("ProcessAsync called with null connection or packet");
                return;
            }
            ...
            try
            {
                ...
                {
                    Logger.Warning($"Rate limit exceeded for {connectionId} - {packetType}");
                ...
                {
                    Logger.Warning($"No handler found for packet type: {packetType} from {connectionId}");

                    if (packet.CorrelationId.HasValue)
                    {
                        var errorResponse = new ErrorPacket(ResponseCode.NETWORK_INVALID_PACKET, $"No handler for packet type: {packetType}")
                        {
                ...
                {
                    Logger.Warning($"Unauthenticated client {connectionId} tried to send {packetType}");

                    var authErrorResponse = new ErrorPacket(ResponseCode.ACCOUNT_NOT_AUTHENTICATED, "Authentication required")
                    {
                        CorrelationId = packet.CorrelationId,
                    };
                ...
                Logger.Debug($"Processing {packetType} from {connectionId}");
                ...
                if (response != null)
                {
                    ...
                    Logger.Debug($"Sent {response.Type} response to {connectionId}");
                }

                Logger.Debug($"Successfully processed {packetType} from {connectionId}");
            }
            catch (Exception ex)
            {
                Logger.Error($"Error processing {packetType} from {connectionId}: {ex.Message}");
                ...
                try
                {
                    if (packet.CorrelationId.HasValue)
                    {
                        var errorResponse = new ErrorPacket(ResponseCode.INTERNAL_ERROR, "An error occurred processing your request")
                        {
                            CorrelationId = packet.CorrelationId
                        };
                        await connection.SendPacketAsync(errorResponse).ConfigureAwait(false);
                ...
                {
                    Logger.Error($"Failed to send error response to {connectionId}: {sendEx.Message}");
                }
            }
        }
        ...
        private void RegisterHandlerMethods(BaseHandler handler)
        {
            ...
            foreach (var method in methods)
            {
                ...
                try
                {
                    ...
                    {
                        Logger.Info($"Registered handler: {handlerType.Name}.{method.Name} for {attribute.PacketType}");
                    }
                    else
                    {
                        Logger.Warning($"Duplicate handler for {attribute.PacketType}: {handlerType.Name}.{method.Name}");
                    }
                }
                catch (Exception ex)
                {
                    Logger.Error($"Failed to register handler {handlerType.Name}.{method.Name}: {ex.Message}");
                }
            }
        }
        ...
        private async Task<BasePacket?> InvokeHandlerAsync(HandlerMethod handlerMethod, BasePacket packet, string connectionId)
        {
            try
            {
                ...
                Logger.Error($"Handler method returned unexpected type: {result?.GetType()}");
                return null;
            }
        ...
        private static (int maxPackets, int timeWindowSeconds) GetRateLimits(PacketType packetType)
        {
            return packetType switch
            {
                ...
                PacketType.PlayerPosition => (Shared.Constants.Network.MaxPositionUpdatesPerSecond, 1),
